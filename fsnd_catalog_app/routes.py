from flask import (render_template, request, flash,
                   url_for, redirect, g, jsonify, session)
from fsnd_catalog_app import app, bcrypt
from fsnd_catalog_app.models import Category, Item, User
from fsnd_catalog_app.forms import (SignupForm, LoginForm, AddCategoryForm,
                                    EditCategoryForm, DeleteCategoryForm,
                                    AddItemForm, EditItemForm, DeleteItemForm)
...
def catalog():
    ...
    return render_template("home.html", categories=all_categories,
                           items=all_items, selected_category=None,
...
def catalog_category(selected_category_id):
    ...
    :param selected_category_id: Integer id identifying
    category in the database.
    :return: Rendered template of homepage showing items
    of only selected category.
    """
    all_categories = Category.query.order_by(Category.id.desc()).all()
    selected_category = Category.query\
        .filter_by(id=selected_category_id).first()
    items_of_selected_category = Item.query.order_by(Item.id.desc())\
        .filter_by(cat_id=selected_category.id)
    number_of_items = len(list(items_of_selected_category))
    return render_template("home.html", categories=all_categories,
                           items=items_of_selected_category,
...
def gconnect():
    ...
    if not User.query.filter_by(username=data['name']).first():
        ...
        User.add_user(google_auth=True, username=data['name'],
                      email=data['email'])
...
def signup():
    ...
    :return: If form submitted, run logic to process sign up.
    If successful, redirect to home page.
    If form not submitted, returns rendered sign up form.
    """
...
def login():
    ...
    :return: If form submitted, process login and if successful,
...
def add_category():
    ...
    :return: If form submitted and validated, add category and
    redirect to home page. Otherwise, return rendered add category form.
    """
    ...
    if form.validate_on_submit():
        ...
        flash('You have successfully added "{}" category'
              .format(form.category_name.data))
        return redirect(url_for("catalog"))
    return render_template("add_category.html", form=form)


@app.route("/catalog_categories/<int:category_id>/edit",
           methods=['GET', 'POST'])
@login_required
def edit_category(category_id):
    """
    View function of edit category form.
    :param category_id: Integer identifying the category being
    edited in our database
    :return: If form submitted and validation successful, save edited
    category and redirect to home page. Otherwise return rendered
    edit category form.
    """
    form = EditCategoryForm()
    category = Category.query.filter_by(id=category_id).first()
    if form.validate_on_submit():
        # If the category doesn't belong to the user, they shouldn't
        # be able to edit it
        if category.creator == current_user:
            ...
            flash("You cannot edit this category because you are "
                  "not its creator.")
        return redirect(url_for('catalog'))
    return render_template("category_edit.html", form=form, category=category)


@app.route("/catalog_categories/<int:category_id>/delete",
           methods=['GET', 'POST'])
@login_required
def delete_category(category_id):
    """
    View function to confirm delete category page.
    :param category_id: Integer identifying category to be
    deleted from database
    :return: If form submitted and authorization sufficient,
    delete category and redirect to home page.
    Otherwise, return rendered template.
    """
    form = DeleteCategoryForm()
    category = Category.query.filter_by(id=category_id).first()
    if form.validate_on_submit():
        # If user is not the creator of the category, they
        ...
        if category.creator == current_user:
            ...
            flash("You cannot delete this category because you "
                  "are not its creator.")
        return redirect(url_for('catalog'))
...
def add_item():
    ...
    :return: If form submitted and validation successful, add item
    to database and redirect to home page.
    Otherwise, return rendered template of add-item form.
    """
    form = AddItemForm()
    if form.validate_on_submit():
        category = Category.query.filter_by(
            name=form.item_category.data).first()
        # remember to store the user_id so that Edit and Delete
        # operations can confirm the creator of the item
        Item.add_item(name=form.item_name.data,
                      description=form.item_details.data,
                      category=category,
                      user_id=current_user.id)
        flash('You have successfully added the item "{}"'
              .format(form.item_name.data))
        return redirect(url_for('catalog'))
...
def show_item_details(selected_item_id):
    ...
    :param selected_item_id: Integer identifying
    selected item in database.
    :return:
    """
...
@app.route("/catalog_items/<int:selected_item_id>/edit",
           methods=['GET', 'POST'])
@login_required
def edit_item(selected_item_id):
    ...
    :return: If form submitted and form validation successful, save the
    changes made to the item in database and redirect to home page.
    Otherwise, return rendered item-edit page.
    """
    form = EditItemForm()
    selected_item = Item.query.filter_by(id=selected_item_id).first()
    if form.validate_on_submit():
        ...
        if selected_item.user_id == current_user.id:
            ...
            Item.edit_item(item_to_edit=selected_item,
                           ...
                           description=form.item_details.data,
                           category=category)
            flash('You have successfully edited {} item'
                  .format(form.item_name.data))
        else:
            flash('You cannot edit item "{}" because you didn\'t create it'
                  .format(selected_item.name))
        return redirect(url_for("catalog"))
    return render_template("item_edit.html", form=form, item=selected_item)


@app.route("/catalog_items/<int:selected_item_id>/delete",
           methods=['GET', 'POST'])
@login_required
def delete_item(selected_item_id):
    """
    View function to delete-item page.
    :param selected_item_id: Integer identifying the item to
    be deleted in database.
    :return: If form submitted and authorization sufficient, delete item,
    ...
    if request.method == 'DELETE':
        if selected_item.user_id == current_user.id:
            ...
            flash('You cannot delete item "{}" because you didn\'t create it'
                  .format(selected_item.name))
        return redirect(url_for('catalog'))
    return render_template("item_confirm_delete.html", form=form,
                           item=selected_item)
...
def full_catalog_json():
    """
    Function to get JSON representatio of full of catalog(that is, all
...
def full_category_json(category_id):
    """
    Function to get JSON of selected category and all items in it
